...
<form method="post" action="index.php?view=report_venta" >
                                <div class="tile-body">
                                    ...
                                    <h4 class="custom-font"><strong>Selecciona</strong> Producto</h4>
                                    <select class="form-control select2" required  name="id_producto"> 
                                    <option value="">--- Selecciona ---</option>  
                                      <?php $productos = ProductoData::getAll();?>
                                      <?php foreach($productos as $producto):?> 
                                      <?php if($producto->marca!='NULL'){ $marca=$producto->marca; }else{ $marca=""; } ?>
                                      <?php if($producto->descripcion!='NULL'){ $descripcion=$producto->descripcion; }else{ $marca=""; } ?>
                                      
                                        <option value="<?php echo $producto->id;?>" ><?php echo $producto->nombre.' '.$marca.' '.$descripcion;?></option>
                                      <?php endforeach;?>
                                    </select> 
